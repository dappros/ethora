...
const MessageDefault: React.FC<IMessagesProps> = ({message, previousJID, nextJID}) => {
    ...
    const [messageDirection, setMessageDirection] = useState<IMessageDirection>({direction: "outgoing"})
    const [userAvatarLink, setUserAvatarLink] = useState<string>("https://icotar.com/initials/" + message.data.senderFirstName + "%20" + message.data.senderLastName);
    ...
    useEffect(() => {
        setMessagePositionData(getPosition(previousJID?.split("/")[0], nextJID?.split("/")[0], message.data.senderJID?.split("/")[0]));
        setMessageDirection({direction: xmpp.client.jid?.toString().split("/")[0] === message.data.senderJID.split("/")[0] ? "outgoing" : "incoming"});
        getImageLink();
    }, []);
    ...
    return (
        <Message
            ...
            avatarSpacer={messagePositionData.type !== 'first' && messagePositionData.type !== 'single'}>

            {messagePositionData.type === 'first' || messagePositionData.type === 'single' ?
                <Avatar src={userAvatarLink} name={message.data.senderFirstName}/> : null
            }
            ...
            {messagePositionData.type === 'last' || messagePositionData.type === 'single' ?
