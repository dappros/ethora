...
import { IApplicationConfig } from "./store_/application"
import qs from "qs"
import type { Stripe } from "stripe"
import { http } from "./api/interceptors_"
...
export const loginUsername = (username: string, password: string) => {
  const appToken = useZustandStore.getState().applicationConfig.appToken
...
export function getConfig(domainName = "ethora") {
  return http.get<{ result: Omit<IApplicationConfig, "firebaseConfig"> }>(
...
export function updateUserAcl(userId: string, appid: string, body: IAclBody) {
  const owner = useZustandStore.getState().user
