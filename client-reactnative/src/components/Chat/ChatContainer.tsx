...
import { audioMimetypes, imageMimetypes, pdfMimemtype, videoMimetypes } from '../../constants/mimeTypes';
...
import { IMessageToSend, createMainMessageForThread } from '../../helpers/chat/createMessageObject';
import { isComposing, pausedComposing, retrieveOtherUserVcard, sendInvite, sendMediaMessageStanza, sendMessageStanza, sendReplaceMessageStanza } from '../../xmpp/stanzas';
...
const ChatContainer = observer((props: ChatContainerProps) => {
    ...
    const [isNftItemGalleryVisible, setIsNftItemGalleryVisible] = React.useState<boolean>(false);
    const [text, setText] = React.useState<string>('');
    const [selection, setSelection] = React.useState({start: 0, end: 0});
    const [isEditing, setIsEditing] = React.useState(false);
    const [onTapMessageObject, setOnTapMessageObject] = React.useState<IMessage>();
    const [isTyping, setIsTyping] = React.useState<boolean>(false);
    const [composingUsername, setComposingUsername] = React.useState<string>('');
    const [showQrModal, setShowQrModal] = React.useState(false);
    const [isShowDeleteOption, setIsShowDeleteOption] = React.useState(true);
    const [showReplyOption, setShowReplyOption] = React.useState(true);
    const [showViewThread, setShowViewThread] = React.useState(false);
    const [showEditOption, setShowEditOption] = React.useState(true);
    const [showMetaNavigation, setShowMetaNavigation] = React.useState(true);
    const [showInChannel, setShowInChannel] = React.useState<boolean>(false);
    ...
    const [dataForLongTapModal, setDataForLongTapModal] = React.useState<
    ...
    const manipulatedWalletAddress = underscoreManipulation(senderDetails.senderWalletAddress);
    ...
    const submitMediaMessage = (mediaListArray: IMediaProps[], roomDetail:roomListProps, senderDetails:ISenderDetails, xmpp:any, waveForm?: any) => {
    ...
    const handleSendMessage = (message:IMessage[]) =>{
        ...
        if(isEditing){
            ...
            const text = parseValue(messageText as string, partTypes as any).plainText;
    ...
    const sendNftItemsFromGallery = (item:any) => {
      ...
      const onMediaMessagePress = (type: any, url: any, message:any) => {
        ...
        const handleOnLongPress = (message: any) => {
            if (
            message.user._id.includes(manipulatedWalletAddress)
            ) {
            return;
            }
            if (
            ...
            if (isImageMimetype(mimetype)) {
              return (
                <ImageMessage
                  ...
                  size={size}
                  onPress={() =>
                    onMediaMessagePress(mimetype, image, props)
                  }
                />
              );
    ...
    return (
        <>
          ...
          source={{uri: roomDetails.roomBackground}}>
              ...
              textInputProps={{
                  color: 'black',
                  onSelectionChange: (e: { nativeEvent: { selection: any; }; }) => setSelection(e.nativeEvent.selection),
              }}
