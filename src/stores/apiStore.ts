import { APP_TOKEN } from "../../docs/config"
import {makeAutoObservable} from 'mobx';

export class ApiStore {
    modes = {
        dev: 'https://app-dev.dappros.com/v1',
        prod: 'https://app.dappros.com/v1',
        qa: 'https://app-dev.dappros.com/v1',
    }

    tokens = {
        dev: APP_TOKEN,
        prod: 'JWT eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..V9Q7uqfG6h__GI3yRD-omjYZj-eD-O2RuhWgUieboEk',
    }

    xmppDomains= {
        DOMAIN: 'dev.dxmpp.com',
        SERVICE: 'wss://dev.dxmpp.com:5443/ws',
        CONFERENCEDOMAIN: '@conference.dev.dxmpp.com',
        CONFERENCEDOMAIN_WITHOUT: 'conference.dev.dxmpp.com',
    }

    defaultUrl= 'https://app-dev.dappros.com/v1'
    defaultToken= APP_TOKEN;
    stores = {}

    constructor(stores:any){
        makeAutoObservable(this)
        this.stores = stores
    }

    changeApiMode(mode:string){
        this.defaultUrl = this.modes[mode];
    }

    changeToken(token:any){
        this.defaultToken = this.tokens[token];
    }

    changeXmpp(xmppDomain:any){
        this.xmppDomains = {
            DOMAIN: xmppDomain,
            SERVICE: `wss://${xmppDomain}:5443/ws`,
            CONFERENCEDOMAIN: '@conference.' + xmppDomain,
            CONFERENCEDOMAIN_WITHOUT: 'conference.' + xmppDomain,
        }
    }
}