...
import {wordpressLoginAction, loginUser, registerUser} from '../actions/auth';
...
class Login extends Component {
  ...
  async componentDidMount() {
    const date = new Date();
    const version = `${date.getFullYear().toString().substring(2)}.${date
      .getMonth()
  ...
  async onFacebookButtonPress() {
    ...
    // Sign-in the user with the credential
    await this.loginOrRegisterSocialUser(user, user.email, 'facebook');
  }
