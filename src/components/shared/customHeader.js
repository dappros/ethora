...
import {commonColors, textStyles, appVersion} from '../../../docs/config';
  ...
  componentDidMount() {
    if (this.props.title) {
      ...
      let chatRoomDetails = this.props.ChatReducer.chatRoomDetails
        ? this.props.ChatReducer.chatRoomDetails
        : this.state.chatRoomDetails;
      this.setState({
        chatRoomDetails,
      });
    }
    ...
    if (this.props.ChatReducer.isRosterUpdated) {
      fetchRosterList().then(rosterListFromRealm => {
        rosterListFromRealm.map(item => {
          if (
            item.jid === this.state.chatRoomDetails.chat_jid &&
            item.name !== this.state.chatRoomDetails.chat_jid
          ) {
            // alert("sdasdcasx")
            this.setState({
              title: item.name,
